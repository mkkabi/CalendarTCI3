...
@Query(value = "SELECT DISTINCT t.* FROM teachers t " +
        "LEFT JOIN disciplines_teachers dt ON dt.teacher_id = t.id " +
        "LEFT JOIN disciplines d ON d.id = dt.discipline_id " +
        "LEFT JOIN groups g ON g.id = d.group_id " +
        "WHERE g.id = ?1 AND t.hidden = 0 " +
        "ORDER BY dt.teacher_id ASC",
        nativeQuery = true)
    List<Teacher> findActiveTeachersForGroup(@Param("groupId") Long groupId);
