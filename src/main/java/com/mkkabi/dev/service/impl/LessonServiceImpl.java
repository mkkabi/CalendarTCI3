...
public class LessonServiceImpl implements LessonService {
    ...
    @Transactional
    public Lesson createForSingleGroup(Lesson lesson, long groupId) {
        try {
            Group group = groupRepository.readById(groupId).orElse(null);
            lesson.setGroups(Collections.singletonList(group));
            Lesson savedLesson = repository.save(lesson);
            logger.info("saved lesson " + savedLesson.getId());
            System.out.println("saved lesson " + savedLesson.getId());
            return savedLesson;
        } catch (IllegalArgumentException e) {
            logger.warning("lesson was null, no lesson created");
            throw new NullEntityReferenceException("Lesson was not created, try again");
