...
public class GroupController {
    ...
    public String read(@PathVariable long id, Model model, @PathVariable Optional<Integer> plusWeek) {
        ...
        Map<String, Integer> lessonCount = new HashMap<>();
        for (LessonDto lesson : weeklyLessons) {
            if(lessonCount.containsKey(lesson.getDiscipline())){
                lessonCount.put(lesson.getDiscipline(), lessonCount.get(lesson.getDiscipline()) + 1);
            }else{
                lessonCount.put(lesson.getDiscipline(), 1);
            }
        }
        ...
        model.addAttribute("totalLessonsNumber", weeklyLessons.size());
